		...
		return mongoConnect()
				.then(function(db){
						...
						myDb.sites = new SmartCollection(
										...
										[
										 { index: { name: 1}, options: { name: 'key', unique: true } }
										 ]);
						// pages
						myDb.pages = new SmartCollection(
										...
										schema({
												...
												keywords: { type: 'array', required: false, message: 'keywords not valid' }
											}),
										[
										 { index: { siteId: 1, url: 1 }, options: { name: 'key', unique: true } },
										 { index: { siteId: 1, '$**': "text" }, options: { name: 'searchIndex' } }
										 ]);
...
exports.searchPages = function (siteName, expression, limit) {
	...
	.then(function(site){
		...
		console.log("SEARCH: Searching for '" + expression + "' top " + limit + " in " + siteName);
